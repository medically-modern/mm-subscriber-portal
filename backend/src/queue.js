...
// ─── Monday Write Queue ───
// Buffers all Monday.com write operations through a queue with:
// - Automatic retry with exponential backoff on failure
// - Concurrency limit to stay under Monday's rate limits
// - Dead letter queue for failed writes after all retries

const mondayWriteQueue = new Queue("monday-writes", {
  connection,
  defaultJobOptions: {
    attempts: 4,
    backoff: { type: "exponential", delay: 2000 },
    removeOnComplete: { count: 500 },  // Keep last 500 completed for debugging
    removeOnFail: { count: 200 },      // Keep last 200 failed for review
  },
});

// Track worker reference for health checks
let _worker = null;
...
function startWorker(mondayQuery) {
  _worker = new Worker(
    ...
    {
      connection,
      concurrency: 3,             // Max 3 concurrent Monday writes
      limiter: {
        max: 10,                  // Max 10 jobs
        duration: 10_000,         // per 10 seconds (stays well under Monday's limits)
      },
    }
  );
...
// Enqueue a Monday write operation
...
// Wait for a write to complete (for user-facing operations that need confirmation)
async function enqueueWriteAndWait(query, variables, label = "write", timeoutMs = 15000) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (state === "failed") {
      ...
      throw new Error(`Write failed: ${label}`);
    }
    await new Promise((r) => setTimeout(r, 250));
  }
  // If timeout, the job will still process in background
